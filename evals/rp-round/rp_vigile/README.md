...
| zone without a society (`blackmarket`, ...) | "corpo": the platform | `rp_economy:add` only; the cut is minted into the `vigile` society. |
...
```text
[rp_vigile] started: 6 templates, 20 €$/min (16 €$ guard + 4 €$ company), bodyguard range 15 m, expel 20 m past the ring
...
[rp_vigile] contract 1 guard 3 expelled 4 from afterlife to 391.2 -2390.0 182.3 (settled)
...
Everything on foot from the **freeroam spawn** `381.36, -2401.79, 181.99` (`VigileConfig.testSpots`).
The zones come from `rp_zones` (`shared/config.lua` there); their centres are copied into
`VigileConfig.zoneGeometry` for the expulsion point:

| Zone | Centre | Radius | Client | From spawn |
|---|---|---|---|---|
| `afterlife` | 360, -2390, 182 | 10 m | `barman` society | 24 m north-west |
| `blackmarket` | 400, -2390, 182 | 10 m | corpo | 22 m north-east |
| `mecano_shop` | 341, -2401, 180.3 | 10 m | `mecano` society | 40 m west |
| `nomad_camp` | 420, -2378, 182 | 9 m | `nomade` society | 46 m north-east |
| `hospital` | 400, -2366, 182 | 12 m | `trauma` society | 41 m north |
| `scrapyard` | 462, -2352, 178 | 12 m | `ferrailleur` society | 95 m north-east |
...
`[rp_vigile] started: 6 templates, ...` then `[rp_vigile] store=sql table=rp_vigile_contracts next_id=1`.

1. **Console:** `setjob 1 barman 3` (seeds the bar's society with 50 000 €$ — the Afterlife's
   client; skip it and take the **blackmarket** contract instead if you want the platform to pay),
   then `setjob 1 vigile 0`. Chat confirms the job (`/job` → `Security guard (vigile) - grade 0/3 recruit`).
2. `/service` → `Clocked in at Security guard as recruit.`
3. `/garde` → the **Security contracts** board: *Guard Afterlife — 30 min - client: the bar*,
   *Guard Black Market ...*, ... Pick **Guard Afterlife** → chat `Contract #1: guard Afterlife for
   30 min at 20 €$/min (16 €$ to you, 4 €$ to the company). Stay inside the ring; ...`, log
   `contract 1 started kind=zone zone=afterlife guard=1 ...`.
4. Walk 24 m north-west to the ring at `360, -2390` (toast **Afterlife**). `/garde` →
   `Contract #1: guarding Afterlife - 30/30 minute(s) left, 0 paid, 0 €$ earned (16 €$/min).` and
   `You are inside the zone: the minute counts.`
...
6. Step 15 m out of the ring: `You left Afterlife: the clock keeps running, but a minute spent
   outside is not paid.` Step back in (toast). `/garde journal` → `Camera log of Afterlife (last 3,
...
12. `/expulser 2` inside the ring → player 2 lands ~20 m past the ring and reads `Security threw
    you out of Afterlife. Don't come back, choom.`; `/garde journal` has `... was thrown out by
