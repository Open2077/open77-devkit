...
local function performAction(medic, kind, target, range)
    ...
    busy[medic] = true
    local promise, dispatchError = Open77.exports.call("open77_uikit", "progress", medic, {
        label = label,
        duration = duration,
        position = "bottom",
        cancellable = true,
    ...
    if promise then
        ...
        -- No UI kit: keep the beat with a plain wait so the act still takes its time.
        log("uikit progress unavailable (%s): plain wait", tostring(dispatchError))
        Wait(duration)
        busy[medic] = nil
    end
